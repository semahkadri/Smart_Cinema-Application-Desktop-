...
QVector<double> SecDialog::Statistique()
{
    ...
    stat[3]=0;
    stat[4]=0;
    q.prepare("SELECT ID_RESERVATION FROM reservation WHERE ID_RESERVATION<=5");
    q.exec();
    while (q.next())
    {
        stat[0]++;
    }
    q.prepare("SELECT ID_RESERVATION FROM reservation WHERE ID_RESERVATION>5 and ID_RESERVATION<=15");
    q.exec();
    while (q.next())
    {
        stat[1]++;
    }
    q.prepare("SELECT ID_RESERVATION FROM reservation WHERE ID_RESERVATION>15 and ID_RESERVATION<=25");
    q.exec();
    while (q.next())
    {
        stat[2]++;
    }
    q.prepare("SELECT ID_RESERVATION FROM reservation WHERE ID_RESERVATION>25 and ID_RESERVATION<=35");
    q.exec();
    while (q.next())
    {
        stat[3]++;
    }
    q.prepare("SELECT ID_RESERVATION FROM reservation WHERE ID_RESERVATION>35 ");
    q.exec();
    while (q.next())
    {
        stat[4]++;
